...
async function deleteUser() {
  ...
  console.log(`Deleted review data for ${userId}.`);
}

async function status() {
  const userId = assertUuid(requiredOption("user-id"), "--user-id");
  const [users, tokens, sessions, syncs, queue] = await Promise.all([
  ...
  console.log(JSON.stringify({ user:users[0] || null, extensionTokens:tokens, sessions, subscriptionSyncs:syncs, queueItemCount:queue.length }, null, 2));
}
...
if (!command || !commands[command]) {
  console.error("Usage: npm run admin -- <token:create|token:revoke|session:revoke|queue:clear|user:delete|user:status|keys:generate> [options]");
  process.exitCode = 1;
} else {
  Promise.resolve()
